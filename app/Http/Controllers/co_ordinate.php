        ...
        foreach($evnt as $e){
            ...
            if($rr==0)
            {
                ...
                if($exist==0)
                {
                    ...
                    if(isset($late_res))
                    {
                        (new self)->notice('Result Not Announced','System','System','student-admin','Result for Event <b>'.ucfirst($e['ename']).'</b> not declared  by co-ordinator <b>'.ucfirst($co['cname']).'</b>','',Session::get('cclgcode'));
                    }
                }
            }
        }
   ...
   public function create_event(Request $req)
    ...
    //  }
     ...
     public function send_notice(Request $req)
     {
        ...
        $this->notice($topic,Session::get('cname'),'co-ordinator','student',$message,$fname,Session::get('cclgcode'));
